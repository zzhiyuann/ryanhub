...
final class BoboViewModel {
    ...
    func generateNudges() async {
        ...
        do {
            ...
            for nudgeData in result.nudges {
                ...
                // Send local push notification for all nudges
                sendNudgeNotification(nudge)
            }

            saveNudges()

            // Post combined nudge content to Chat as a Bo message
            if !result.nudges.isEmpty {
                let combined = result.nudges.map(\.content).joined(separator: "\n\n")
                NotificationCenter.default.post(
                    name: .boNudgeGenerated,
                    object: nil,
                    userInfo: ["content": combined]
                )
            }
    ...
    func fetchHealthKitEvents(completion: (() -> Void)? = nil) {
        ...
        // Heart rate — full data, grouped by minute
    ...
    func pullNarrationsFromServer() async {
        ...
        guard let url = URL(string: "\(baseURL)/bobo/narrations") else { return }
        ...
        do {
            ...
            // Merge: add any narrations not already in local store
            let existingIDs = Set(narrations.map(\.id))
            let newNarrations = serverNarrations.filter { !existingIDs.contains($0.id) }
            if !newNarrations.isEmpty {
                narrations.append(contentsOf: newNarrations)
    ...
    // MARK: - Pull Nudges from Server

    /// Pull nudges generated by the bridge server (e.g. background nudge-check).
    /// Merges by ID and posts new nudges to Chat.
    func pullNudgesFromServer() async {
        let baseURL = Self.bridgeBaseURL
        guard let url = URL(string: "\(baseURL)/bobo/nudges") else { return }
        ...
        do {
            ...
            decoder.keyDecodingStrategy = .convertFromSnakeCase
            decoder.dateDecodingStrategy = .iso8601
            let serverNudges = try decoder.decode([Nudge].self, from: data)

            // Merge: add any nudges not already in local store
            let existingIDs = Set(nudges.map(\.id))
            let newNudges = serverNudges.filter { !existingIDs.contains($0.id) }
            if !newNudges.isEmpty {
                nudges.insert(contentsOf: newNudges, at: 0)
                saveNudges()

                // Post new nudges to Chat
                let combined = newNudges.map(\.content).joined(separator: "\n\n")
                NotificationCenter.default.post(
                    name: .boNudgeGenerated,
                    object: nil,
                    userInfo: ["content": combined]
                ...
                print("[BoBo] Pulled \(newNudges.count) new nudges from server")
            }
        } catch {
            print("[BoBo] Failed to pull nudges from server: \(error.localizedDescription)")
        }
    }
...
// MARK: - Bo Nudge Notification

extension Notification.Name {
    static let boNudgeGenerated = Notification.Name("boNudgeGenerated")
}
