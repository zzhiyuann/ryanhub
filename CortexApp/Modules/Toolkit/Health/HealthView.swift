...
struct HealthView: View {
    ...
    private var weightContent: some View {
        VStack(spacing: CortexLayout.sectionSpacing) {
            ...
            // Log button
            CortexButton("Log Weight", icon: "plus.circle.fill") {
                showWeightLog = true
            }
    ...
    private func weightChangeBadge(change: Double) -> some View {
        let isGain = change > 0
        let color: Color = isGain ? .cortexAccentRed : .cortexAccentGreen
    ...
    private var miniWeightChart: some View {
        ...
        return VStack(spacing: 4) {
            GeometryReader { geometry in
                ...
                .stroke(Color.cortexPrimary, style: StrokeStyle(lineWidth: 2, lineCap: .round, lineJoin: .round))
    ...
    private var foodContent: some View {
        VStack(spacing: CortexLayout.sectionSpacing) {
            ...
            // Log button
            CortexButton("Log Meal", icon: "plus.circle.fill") {
                showFoodLog = true
            }
    ...
    private func foodEntryRow(_ entry: FoodEntry) -> some View {
        HStack(spacing: 12) {
            Image(systemName: entry.mealType.icon)
                ...
                .background(
                    Circle().fill(Color.cortexPrimary.opacity(0.12))
                )
            ...
            if let calories = entry.calories {
                Text("\(calories) cal")
                    .font(.system(size: 13, weight: .semibold))
                    .foregroundStyle(Color.cortexAccentYellow)
            }
        }
        .padding(CortexLayout.cardInnerPadding)
        .background(
    ...
    private var activityContent: some View {
        VStack(spacing: CortexLayout.sectionSpacing) {
            ...
            // Log button
            CortexButton("Log Activity", icon: "plus.circle.fill") {
    ...
    private func activityEntryRow(_ entry: ActivityEntry) -> some View {
        HStack(spacing: 12) {
            Image(systemName: "figure.run")
                ...
                .frame(width: 36, height: 36)
                .background(
                    Circle().fill(Color.cortexAccentGreen.opacity(0.12))
                )
            ...
            ScrollView {
                VStack(spacing: CortexLayout.sectionSpacing) {
                    VStack(alignment: .leading, spacing: CortexLayout.itemSpacing) {
                        ...
                        CortexTextField(placeholder: "e.g., Running, Walking, Gym", text: $activityType)
                    }

                    VStack(alignment: .leading, spacing: CortexLayout.itemSpacing) {
                        SectionHeader(title: "Duration (minutes)")
                        CortexTextField(placeholder: "30", text: $durationText)
                    ...
                    VStack(alignment: .leading, spacing: CortexLayout.itemSpacing) {
                        ...
                        CortexTextField(placeholder: "Add a note...", text: $note)
                    }
