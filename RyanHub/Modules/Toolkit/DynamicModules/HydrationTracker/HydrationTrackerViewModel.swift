...
// MARK: - HydrationTracker View Model
...
final class HydrationTrackerViewModel {
    ...
    func loadData() async {
        ...
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/hydrationTracker/data")!
            let (data, _) = try await URLSession.shared.data(from: url)
            entries = try JSONDecoder().decode([HydrationTrackerEntry].self, from: data)
    ...
    func addEntry(_ entry: HydrationTrackerEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/hydrationTracker/data/add")!
    ...
    func deleteEntry(_ entry: HydrationTrackerEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/hydrationTracker/data?id=\(entry.id)")!
            var request = URLRequest(url: url)
