...
// MARK: - GratitudeJournal View Model
...
final class GratitudeJournalViewModel {
    ...
    func loadData() async {
        ...
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/gratitudeJournal/data")!
            let (data, _) = try await URLSession.shared.data(from: url)
            entries = try JSONDecoder().decode([GratitudeJournalEntry].self, from: data)
            UserDefaults.standard.set(data, forKey: "dynamic_module_gratitudeJournal_cache")
        } catch {
            entries = []
        }
    }

    func addEntry(_ entry: GratitudeJournalEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/gratitudeJournal/data/add")!
    ...
    func deleteEntry(_ entry: GratitudeJournalEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/gratitudeJournal/data?id=\(entry.id)")!
            var request = URLRequest(url: url)
