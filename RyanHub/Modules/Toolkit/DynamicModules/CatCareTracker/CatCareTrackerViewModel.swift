...
// MARK: - CatCareTracker View Model
...
final class CatCareTrackerViewModel {
    ...
    func loadData() async {
        ...
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/catCareTracker/data")!
            let (data, _) = try await URLSession.shared.data(from: url)
            entries = try JSONDecoder().decode([CatCareTrackerEntry].self, from: data)
    ...
    func addEntry(_ entry: CatCareTrackerEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/catCareTracker/data/add")!
    ...
    func deleteEntry(_ entry: CatCareTrackerEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/catCareTracker/data?id=\(entry.id)")!
            var request = URLRequest(url: url)
