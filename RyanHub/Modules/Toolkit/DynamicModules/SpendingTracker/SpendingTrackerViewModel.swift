...
// MARK: - SpendingTracker View Model
...
final class SpendingTrackerViewModel {
    ...
    func loadData() async {
        ...
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/spendingTracker/data")!
            let (data, _) = try await URLSession.shared.data(from: url)
            entries = try JSONDecoder().decode([SpendingTrackerEntry].self, from: data)
    ...
    func addEntry(_ entry: SpendingTrackerEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/spendingTracker/data/add")!
    ...
    func deleteEntry(_ entry: SpendingTrackerEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/spendingTracker/data?id=\(entry.id)")!
            var request = URLRequest(url: url)
