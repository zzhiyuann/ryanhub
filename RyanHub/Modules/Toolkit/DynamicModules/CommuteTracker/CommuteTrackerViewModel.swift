...
// MARK: - CommuteTracker View Model
...
final class CommuteTrackerViewModel {
    ...
    var todayEntries: [CommuteTrackerEntry] {
    ...
    func loadData() async {
        ...
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/commuteTracker/data")!
            let (data, _) = try await URLSession.shared.data(from: url)
            entries = try JSONDecoder().decode([CommuteTrackerEntry].self, from: data)
    ...
    func addEntry(_ entry: CommuteTrackerEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/commuteTracker/data/add")!
    ...
    func deleteEntry(_ entry: CommuteTrackerEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/commuteTracker/data?id=\(entry.id)")!
            var request = URLRequest(url: url)
