...
final class SleepTrackerViewModel {
    ...
    var todayEntries: [SleepTrackerEntry] {
    ...
    func loadData() async {
        ...
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/sleepTracker/data")!
            let (data, _) = try await URLSession.shared.data(from: url)
            entries = try JSONDecoder().decode([SleepTrackerEntry].self, from: data)
    ...
    func addEntry(_ entry: SleepTrackerEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/sleepTracker/data/add")!
    ...
    func deleteEntry(_ entry: SleepTrackerEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/sleepTracker/data?id=\(entry.id)")!
            var request = URLRequest(url: url)
