...
struct ReadingTrackerEntrySheet: View {
    ...
    @State private var inputPagesread: Int = 1
    ...
    var body: some View {
        QuickEntrySheet(
            ...
            canSave: true,
            onSave: {
                let entry = ReadingTrackerEntry(bookTitle: inputBooktitle, author: inputAuthor, totalPages: inputTotalpages, currentPage: inputCurrentpage, pagesRead: inputPagesread, minutesRead: inputMinutesread, status: selectedStatus, genre: selectedGenre, rating: inputRating, notes: inputNotes)
                ...
                EntryFormSection(title: "Pages Read (Session)") {
                    Stepper("\(inputPagesread) pages read (session)", value: $inputPagesread, in: 0...9999)
                }
                ...
                EntryFormSection(title: "Session Notes") {
                    HubTextField(placeholder: "Session Notes", text: $inputNotes)
                }
        }
    }
}
