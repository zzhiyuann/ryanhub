...
final class HabitTrackerViewModel {
    ...
    func loadData() async {
        ...
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/habitTracker/data")!
            let (data, _) = try await URLSession.shared.data(from: url)
            entries = try JSONDecoder().decode([HabitTrackerEntry].self, from: data)
    ...
    func addEntry(_ entry: HabitTrackerEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/habitTracker/data/add")!
    ...
    func deleteEntry(_ entry: HabitTrackerEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/habitTracker/data?id=\(entry.id)")!
            var request = URLRequest(url: url)
