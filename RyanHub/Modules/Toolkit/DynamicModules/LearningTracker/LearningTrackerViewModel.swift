...
// MARK: - LearningTracker View Model

@Observable
@MainActor
final class LearningTrackerViewModel {
    var entries: [LearningTrackerEntry] = []
    var isLoading = false
    ...
    var todayEntries: [LearningTrackerEntry] {
    ...
    func loadData() async {
        ...
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/learningTracker/data")!
            let (data, _) = try await URLSession.shared.data(from: url)
            entries = try JSONDecoder().decode([LearningTrackerEntry].self, from: data)
            UserDefaults.standard.set(data, forKey: "dynamic_module_learningTracker_cache")
        } catch {
            entries = []
        }
    }

    func addEntry(_ entry: LearningTrackerEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/learningTracker/data/add")!
    ...
    func deleteEntry(_ entry: LearningTrackerEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/learningTracker/data?id=\(entry.id)")!
            var request = URLRequest(url: url)
