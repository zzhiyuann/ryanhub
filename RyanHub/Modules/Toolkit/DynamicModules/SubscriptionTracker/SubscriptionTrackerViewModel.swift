...
// MARK: - SubscriptionTracker View Model

@Observable
@MainActor
final class SubscriptionTrackerViewModel {
    var entries: [SubscriptionTrackerEntry] = []
    var isLoading = false
    ...
    var todayEntries: [SubscriptionTrackerEntry] {
    ...
    func loadData() async {
        ...
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/subscriptionTracker/data")!
            let (data, _) = try await URLSession.shared.data(from: url)
            entries = try JSONDecoder().decode([SubscriptionTrackerEntry].self, from: data)
            UserDefaults.standard.set(data, forKey: "dynamic_module_subscriptionTracker_cache")
        } catch {
            entries = []
        }
    }

    func addEntry(_ entry: SubscriptionTrackerEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/subscriptionTracker/data/add")!
    ...
    func deleteEntry(_ entry: SubscriptionTrackerEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/subscriptionTracker/data?id=\(entry.id)")!
            var request = URLRequest(url: url)
