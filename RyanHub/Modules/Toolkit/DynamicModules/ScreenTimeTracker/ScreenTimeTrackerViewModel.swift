...
// MARK: - ScreenTimeTracker View Model
...
final class ScreenTimeTrackerViewModel {
    ...
    func loadData() async {
        ...
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/screenTimeTracker/data")!
            let (data, _) = try await URLSession.shared.data(from: url)
            entries = try JSONDecoder().decode([ScreenTimeTrackerEntry].self, from: data)
    ...
    func addEntry(_ entry: ScreenTimeTrackerEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/screenTimeTracker/data/add")!
    ...
    func deleteEntry(_ entry: ScreenTimeTrackerEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/screenTimeTracker/data?id=\(entry.id)")!
            var request = URLRequest(url: url)
