...
// MARK: - DailyAffirmations View Model

@Observable
@MainActor
final class DailyAffirmationsViewModel {
    var entries: [DailyAffirmationsEntry] = []
    var isLoading = false
    ...
    var todayEntries: [DailyAffirmationsEntry] {
    ...
    func loadData() async {
        ...
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/dailyAffirmations/data")!
            let (data, _) = try await URLSession.shared.data(from: url)
            entries = try JSONDecoder().decode([DailyAffirmationsEntry].self, from: data)
            UserDefaults.standard.set(data, forKey: "dynamic_module_dailyAffirmations_cache")
        } catch {
            entries = []
        }
    }

    func addEntry(_ entry: DailyAffirmationsEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/dailyAffirmations/data/add")!
    ...
    func deleteEntry(_ entry: DailyAffirmationsEntry) async {
        do {
            let url = URL(string: "\(bridgeBaseURL)/modules/dailyAffirmations/data?id=\(entry.id)")!
            var request = URLRequest(url: url)
